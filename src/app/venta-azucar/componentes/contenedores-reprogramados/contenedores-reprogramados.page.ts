...
@Component({
  selector: 'app-contenedores-reprogramados',
  templateUrl: './contenedores-reprogramados.page.html',
  styleUrls: ['./contenedores-reprogramados.page.scss'],
})
export class ContenedoresReprogramadosPage implements OnInit {
  ...
  getData() {
    ...
    Promise.all([this.storage.get('name_tkn'),this.storage.get('id_user')]).then(values => {
      ...
      try {
        this.http.post(environment.url_site+'comercializadora/contenedores/reprogramados', 
  ...
  pendientes() {
    this.navCtrl.navigateRoot('/contenedores-pendientes');
  }
